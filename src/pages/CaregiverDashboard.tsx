...
import { mockPatient, mockAdherence, mockAlerts } from '../data/mockData';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader
        ...
        subtitle={`Monitoring ${mockPatient.name}`}
        action={
        ...
        <GlassCard className="bg-red-50/90 border-red-200 shadow-lg shadow-red-500/10 flex flex-col sm:flex-row items-center sm:items-start gap-4 p-6">
           ...
           <div className="flex-1 text-center sm:text-left">
             ...
             <div className="mt-5 flex flex-wrap justify-center sm:justify-start gap-3">
               <button className="bg-red-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md hover:bg-red-700 transition-colors">Acknowledge Alert</button>
               <button className="bg-white text-red-700 px-5 py-2.5 rounded-xl text-sm font-bold border border-red-200 flex items-center justify-center gap-2 shadow-sm hover:bg-red-50 transition-colors">
                 <Phone className="w-4 h-4"/> Call {mockPatient.name.split(' ')[0]}
               </button>
             </div>
           </div>
        </GlassCard>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
         <GlassCard className="lg:col-span-2 p-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-6 mb-10 pb-8 border-b border-slate-100">
               <div>
                 <div className="text-6xl font-black text-emerald-500 tracking-tighter">{mockAdherence.overallPercentage}%</div>
               ...
               <div className="sm:ml-auto p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-2xl font-bold text-slate-800 mt-1">{mockAdherence.totalForWeek}</p>
               </div>
            </div>
            ...
            <div className="space-y-6">
               <AdherenceBar 
                 label="Doses Taken" 
                 count={mockAdherence.takenCount} 
                 total={mockAdherence.totalForWeek} 
                 colorClass="bg-emerald-500" 
                 textClass="text-emerald-700" 
               />
               <AdherenceBar 
                 label="Missed Doses" 
                 count={mockAdherence.missedCount} 
                 total={mockAdherence.totalForWeek} 
               ...
               <AdherenceBar 
                 ...
                 count={mockAdherence.unconfirmedCount} 
                 total={mockAdherence.totalForWeek} 
         ...
         <div className="space-y-8">
            <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-2xl relative overflow-hidden">
               ...
               <div className="space-y-5 relative z-10">
                  <div>
                     ...
                     <p className="text-xl font-bold">Amlodipine (8:00 PM)</p>
                  </div>
                  <div className="h-px bg-slate-800" />
                  <div>
                     <p className="text-sm text-slate-400 mb-1">Last Application Activity</p>
                     <p className="text-lg font-medium text-slate-200">2 hours ago</p>
                  </div>
                  <div className="h-px bg-slate-800" />
                  <div>
            ...
            <GlassCard className="p-6 bg-primary-light/5 border-primary-light/20">
               ...
               <div className="space-y-3">
                  <button className="group w-full flex items-center justify-between px-5 py-4 bg-white rounded-2xl text-sm font-bold text-slate-700 hover:bg-primary hover:text-white transition-all shadow-sm border border-slate-100">
                    Edit Medicine List
